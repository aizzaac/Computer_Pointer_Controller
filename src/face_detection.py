...
class FaceDetectionClass():
    ...
    def preprocess_output(self, outputs, image, args):
        ...
        - outputs: this is the array returned by the 'predict' function = [1,1,N,7]
        - image: input image (without preprocessing)
        '''
        width = int(image.shape[1])#input image width
        height = int(image.shape[0])#input image height
        
        faces = []

        for i in range(len(outputs[0][0])):
            box = outputs[0][0][i]
            probability = box[2] #only boxes over a certain probability threshold (pt)
            if probability >= args.prob_threshold: #pt is set by the user
                ...
                if args.view == "YES":#if user wants to preview the 'bounding boxes', etc
                    cv2.rectangle(image, (xmin, ymin), (xmax, ymax), (0, 0, 255), 3) #draw red rectangles of width 3
                ##crop_face = image[ymin:ymax, xmin:xmax]#cut the face out of the whole image   
                
                faces.append([xmin, ymin,xmax, ymax])

        return image, faces##, crop_face
